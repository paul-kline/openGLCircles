...
double Controller::getViewportAspectRatio() const
{
	...
	getViewportDimensions(vpWidth, vpHeight);
	return static_cast<double>(vpHeight) / static_cast<double>(vpWidth);
}

void Controller::getViewportDimensions(int& width, int& height) const
{
	int vp[4];
...
void Controller::screenXYToLDS(int x, int y, double& ldsX, double& ldsY)
{
	...
	// of the window. The main OpenGL API assumes y=0 is at the bottom, so note
